...
void drawLine(int x0, int y0, int x1, int y1, byte red, byte green, byte blue){
  ...
  dy <<= 1; // dy is now 2*dy
  dx <<= 1; // dx is now 2*dx
  drawPixel(x0, y0, red, green, blue); // Draw the first pixel.
...
void drawRect(int x0, int y0, int x1, int y1, byte red, byte green, byte blue){
  drawRect(x0, y0, x1, y1, red, green, blue, false);
}

void drawRect(int x0, int y0, int x1, int y1, byte red, byte green, byte blue, boolean fill){
  ...
  if (fill == 1)
  {
    ...
    while(xDiff > 0)
    {
      drawLine(x0, y0, x0, y1, red, green, blue);
  ...
  else 
  {
    ...
    drawLine(x0, y1, x1, y1, red, green, blue);
    drawLine(x0, y0, x0, y1, red, green, blue);
    drawLine(x1, y0, x1, y1, red, green, blue);
  }
}
...
void drawRect_custom( int x0, int y0, int x1, int y1, int x2, int y2, int x3, int y3, byte red, byte green, byte blue, boolean fill){
  if (fill){
    ...
    for (int i_x=min_x; i_x<max_x; i_x++){
      for (int i_y=min_y; i_y<max_y; i_y++){
        ...
        if (
          ...
          drawPixel(i_x, i_y, red, green, blue);
        }
      }
    }    
  }else{
    ...
    drawLine(x3, y3, x0, y0, red, green, blue);
  }
}
        ...
        for (int reading_byte=0; reading_byte<(image_wigth*image_height%8==0?image_wigth*image_height/8:image_wigth*image_height/8+1); reading_byte++){
          ...
          if(current_byte!=0x00 && current_byte!=0xFF){
            for (unsigned char d=0; d<8; d++){
              ...
              if (current_byte&1<<(7-d)) drawPixel(x + icon_x, y + icon_y, red, green, blue);
              icon_x ++;
            }
            ...
            drawLine(x + icon_x, y + icon_y, x + icon_x+7, y + icon_y, red, green, blue); 
            icon_x+=8;
          }else{ // Saving 1ms!!!!
            icon_x+=8;
          }
          ...
          if(fontSize>1){
            for(int j=0; j<fontSize; j++){
              for(int jj=0; jj<fontSize; jj++){
                drawPixel(x + char_part*fontSize + i*FONT_CHAR_WIDTH*fontSize + jj, y + bit*fontSize - j, red, green, blue);
              }
            }
          }else{
            drawPixel(x + char_part + i*FONT_CHAR_WIDTH, y + bit, red, green, blue);
          }
        }
      }
    }
